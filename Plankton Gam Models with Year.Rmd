...
A critical problem that crops up here is that adding year results in some
changes in conclusions when compared to analyses that leave Year out of the
models. This is not entirely unexpected. The problem is that Years vary both in 
plankton composition and abundance and in various predictors.  For example, some 
years are warmer than others.  so, if you fit both "year" and "Temperature" in a 
model, the model ends up partitioning variation between the two predictors. 
...
Note, however, that we have data from only 60 samples.  Each (smoothed) 
predictor could, in principal, enter the model with as many as nine degrees of 
freedom.
...
Turbidity       | Up to 9
Chlorophyll     | Up to 9
River Herring   | Up to 9

So, in principle, we could be fitting models up to 54 degrees of freedom, which
would leave us with few degrees of freedom for error.  Luckily, in
practice, most terms are not important, and they drop out of our models, or fit
smoothed terms (thin plate splines) with much smaller effective degrees of 
freedom.
...
I'm building on the thinking in the other notebook ere, so skipping over basic 
background on GAMS.
...
So, ambiguous.  Looking at results, my tendency is to want to treat year as a 
random factor, effectively treating it as uninteresting year to year variation
that we expect to occur, but that we are uninterested in on its own.
...
values would be better -- or scientifically more relevant -- predictors of 
zooplankton than the transformed ones.  For example, predation intensity is
...
specifies shrinkage estimates using "thin plate" splines.
...
discussed on the call (negative binomial and quasipoisson GAMs) are not going to 
...
3.  Use a Gamma family GLM / GAM -- this implies standard error is roughly 
    ...
    *  an inverse link (related to the "canonical link")
    ...
    expected, usually on scientific grounds. The idnetity link yields an 
    "additive" model, the log link, a "multiplicative model" and the inverse 
    link some sort of a "rate" or "harmonic mean" model.
...
transformed data and the Gamma model with a log link.
...
Note that the linear predictors are important.  Years differ, and conditions at
the Stations differ. 
...
1. Low salinity sample has low abundance.  otherwise, salinity does not much 
   matter, although there may be a remnant effect.
   
2. Abundance increases with Turbidity.

3. Chlorophyll may also matter, but it is only marginally significant here.
...
This model has the best residual structure of any of the models we have looked 
at, but it still has that one extreme low predicted value.... 


## Conclusions from too many GAM Models

All three models are reasonable...
...
2.  That one very low abundance, low salinity sample dominates the relationship
    ...
    if  based mostly on a single sample.
...
## Gamma GAM with Inverse link
```{r}
shannon_gam_g <- gam(H ~ Station + 
                     Yearf +
                     s(Temp, bs="ts") +
                     s(Sal, bs="ts") + 
                     s(log(Turb), bs="ts") + 
                     s(log(Chl), bs="ts") + 
                     s(log1p(RH),bs="ts"),
                   data = base_data, family =Gamma(link = 'inverse'))
summary(shannon_gam_g)
```
The only significant relationship between diversity and any of the predictors 
relates to salinity.  That also holds with several other models, including
gamma models with log and identity links.
...
Again, I suspect the salinity curve is influenced by a handful of low salinity
samples.

```{r fig.width = 5, fig.height = 5}
oldpar <- par(mfrow = c(2,2))
gam.check(shannon_gam_g)
par(oldpar)
```
...
That extreme negative outlier is again the main weakness of this model.
...
that the identity link finds a significant relationship with temperature.
